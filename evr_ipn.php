...
function evr_authorize_txn(){
	...
	$id="";
	//$id=$_REQUEST['id'];//This is the id of the registrant
    $id=$_REQUEST['x_cust_id'];//This is the id of the registrant
	if ($id ==""){
		echo "ID not supplied.";
	}
    /*
        ...
        */
				...
				//store the results in reusable variables
				$payer_id = $id;//required
				$event_id = $_POST['x_invoice_num'];//required
                $payment_date = $today;
				$txn_id = $_POST['x_trans_id'];//required
				$first_name = $_POST['x_first_name'];//required
				$last_name = $_POST['x_last_name'];//required
				$payer_email = $_POST['x_email'];//required
				$payer_status = $_POST['x_response_code'];//required
				
                $payment_type = $_POST['x_method'];//required
                
				$memo = "memo";//required
				$item_name = $_POST['x_description'];//required
				$item_number = $_POST['x_invoice_num'];//required
				$quantity = "1";//required
				$amount_pd = $_POST['x_amount'];//required mc gross
				$mc_currency = "USD";//required
				...
				$phone= $_POST['x_phone'];
				$payment_status = $_POST['x_response_reason_code'];//limited to 17 char - required
				$pending_reason = "pending";//limited to 14 char
				$reason_code =$_POST['x_response_reason_text'];//limited to 15 char
				$txn_type = $_POST['x_type'];//limited to 20 char - required
                
                $payer_business_name = $address_name;
            ...
            $wpdb->query($wpdb->prepare("UPDATE ". $events_attendee_tbl . " SET payment_status = '$payment_status', amount_pd = '$amount_pd', payment_date ='$payment_date' WHERE id = %d",$id));
                ...
                //(is_numeric($_REQUEST['x_invoice_num'])) ? $event_id = $_REQUEST['x_invoice_num'] : $event_id = "0";
                ...
                $sql = "SELECT * FROM ". get_option('evr_event') ." WHERE id = $event_id";	
                    ...
                    $event = $wpdb->get_row($sql);
                            ...
                            $send_coord = $event->send_coord;
                            $coord_email = $event->coord_email;
                            $coord_msg = stripslashes($event->coord_msg);
                            $coord_pay_msg = stripslashes($event->coord_pay_msg);
     ...
     $attendee = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". get_option('evr_attendee') ." WHERE id = %d",$id));
			...
			$headers .= "Reply-To: " . stripslashes($company_options['company_name']) . "  <" . $company_options['company_email'] . ">\r\n";
                ...
                $subject = 'Instant Payment Notification - Success';
				 ...
				 wp_mail($contact, $subject, $body,$headers);
                       ...
                       //$attendee_email   = "consultant@avdude.com";
						...
						$payment_msg      = stripslashes($company_options['payment_message']);
						$pay_confirm      = $company_options['pay_confirm'];
						$Organization     =  stripslashes($company_options['company_name']);
						$contact          = $company_options['company_email'];
			$headers .= "From: " . $Organization . " <". $contact . ">\r\n";
			$headers .= "Reply-To: " . $Organization . "  <" . $contact . ">\r\n";
            if ($send_coord == "Y") {$contact = $coord_email;} else {$contact = $company_options['company_email'];}
