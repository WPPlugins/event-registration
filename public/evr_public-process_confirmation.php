...
function evr_process_confirmation(){
    ...
    if ( ! isset( $_POST['nonce_verification'] ) || ! wp_verify_nonce( $_POST['nonce_verification'], 'nonce_verify' ) ) {
        print 'Sorry, It appears you may be trying to spoof the registration form.  Please retry.';
        exit;
    } 
    else {
   // process form data
    ...
    $reg_form = sanitize_term(unserialize(urldecode($_POST["reg_form"])),'display');
	#GT Hook for MailChimp Addon
	do_action('evr_process_confirmation', $reg_form );
    $qanda = sanitize_term(unserialize(urldecode($_POST["questions"])),'display');
    $attendee_array = sanitize_term($_POST['attendee'], 'display');
    function attendee_filter(&$value) { $value = htmlspecialchars($value, ENT_QUOTES, 'UTF-8');}
    array_walk_recursive($attendee_array, "attendee_filter");

    #We added a session toaken to the confirmation page to eliminate double postings
    $submitted_token = sanitize_text_field(isset($_POST['token'])?$_POST['token']:'0');
...
function evr_show_confirmation()
{
   ...
   //$sql= "SELECT * FROM ". get_option('evr_event')." WHERE id=".$event_id; 
    $_SESSION['event_array'] = $event;
  ?> 
   <span style="float:right;"><a href="<?php echo EVR_PLUGINFULLURL;?>evr_ics.php">
<img src="<?php echo EVR_PLUGINFULLURL;?>images/ical-logo.jpg" /></a></span><br />
<?php
        $event_id = $event->id;
...
?>
<form id="pdf out" class="evr_regform" method="post" action="<?php echo plugins_url('evr_pdf_out.php',__FILE__ );?> ">
...
if ($company_options['evr_invoice'] == "Y"){
    echo '<form id="invoice" class="evr_regform" method="post" target=_blank action="'.plugins_url('tcpdf/examples/invoice.php').'">';
?>
