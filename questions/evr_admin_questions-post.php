...
function evr_post_question(){
    ...
    $event_name = (string)@$_REQUEST['event_name'];
    $question = (string)@$_REQUEST ['question'];
    $question_type = (string)@$_REQUEST ['question_type'];
    $values = (string)@$_REQUEST ['values'];
    $required = (string)@$_REQUEST ['required'] ? 'Y' : 'N';
    $remark = (string)@$_REQUEST['remark'];
...
<?php } 
    ...
    ?>
	<div id="message" class="error"><p><strong><?php _e('There was an error in your submission (No question provided!), please try again. The question was not saved!','evr_language');?><?php print $wpdb->last_error; ?>.</strong></p>
    <p><strong><?php _e(' . . .Now returning you to Question Management . . ','evr_language');?><meta http-equiv="Refresh" content="3; url=admin.php?page=questions&action=new&event_id=<?php echo $event_id;?>&event_name=<?php echo $event_name;?>"></strong></p>
    </div>
<?php 
